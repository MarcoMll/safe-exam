...
while cap.isOpened():
    ...
    # makes it so we can only read the image when passing it
        ...
        for face_landmarks in results.multi_face_landmarks:
            ...
            for idx, lm in enumerate(face_landmarks.landmark):
                ...
                if len(face_2d) >= 6:
                    ...
                    ok, roc_vec, trans_vec = cv2.solvePnP(
                        face_3d,
                        face_2d,
                        cam_matrix,
                        dist_matrix,
                    ...
                    if ok:
                        # convert roc_vec (rotation vector) to rotation matrix
                        # (we will not use jacobian matrix (jac))
                        rmat, jac = cv2.Rodrigues(roc_vec)

                        # get the angles for all the axes
                        # using cv2 RQDecomp3x3 function to decompose
                        # the rotation matrix into angles
                        ...
                        if nose_3d is not None and nose_2d is not None:
                            nose_3d_arr = np.array([nose_3d], dtype=np.float64)
                            nose_2d_arr = np.array([nose_2d], dtype=np.float64)
                        ...
                        # the cordinates and the text of the direction
